...
static inline int fls(int x)
{
    if (x == 0)
        return 0;
    // 32 minus the number of leading zeroes of x
...
static inline int fls64(uint64_t x)
{
    if (x == 0)
        return 0;
    // 64 minus the number of leading zeroes of x
...
static void hb_val_from_type(char *str, void *val, u2 desc_idx, java_class_t *cls) {
    ...
    switch (*desc) {
        ...
        case 'C': // Char
            ...
            break;
        case 'D': // Double
            sprintf(str, "%f", *((double*)&val)); // Assumes d8 is double
            break;
        case 'F': // Float
            sprintf(str, "%f", *((float*)&val)); // Assumes f4 is float
            break;
        case 'I': // Integer

...
sweep (gc_state_t * state)
{
    struct nk_hashtable_iter* iter = nk_create_htable_iter(state->ref_tbl->htable);
    if (!iter) {
        HB_ERR("Could not create an iterator of the reference table\n");
        return -1;
    }
    ...
    while (nk_htable_iter_advance(iter) != 0) { 
        ...
        if (entry && entry->state == GC_REF_ABSENT) {
            ref_tbl_remove_ref((obj_ref_t*)nk_htable_get_iter_key(iter));
            local_count++;
        }
    }
    state->collect_stats.obj_collected += local_count;
...
scan_base_obj (gc_state_t * gc_state, void * priv_data)
{
	...
	u2 obj_fc = obj->field_count;

	for(u2 i = 0; i < obj_fc; i++)
	{
...
scan_base_frame (gc_state_t * gc_state, void * priv_data)
{
    ...
    while (frame) {
        for (u4 i = 0; i < frame->max_locals; i++) {
            obj_ref_t* local_ref = frame->locals[i].obj;
            if (is_valid_ref(local_ref, gc_state)) {
                mark_ref(local_ref, gc_state);
            }
        }

        op_stack_t* op_stack = frame->op_stack;
        for (int sp = op_stack->sp; sp >= 0; sp--) {
            obj_ref_t* stack_ref = op_stack->oprs[sp].obj;
            if (stack_ref && is_valid_ref(stack_ref, gc_state)) { 
                mark_ref(stack_ref, gc_state);
            }
        }
...
scan_class_map (gc_state_t * gc_state, void * priv_data)
{
    ...
    if (!iter) {
        HB_ERR("Error creating ref table iterator during %s\n", __func__);
        return -1;
    }
    ...
    while (nk_htable_iter_advance(iter) != 0) {
        ref_entry_t* entry = (ref_entry_t*)nk_htable_get_iter_value(iter);
        if (entry) {
            java_class_t* class_obj = (java_class_t*)entry;
            for (u2 i = 0; i < class_obj->fields_count; i++) {
                if (class_obj->fields[i].acc_flags & ACC_STATIC && is_valid_ref(class_obj->field_vals[i].obj, gc_state)) {
                    mark_ref(class_obj->field_vals[i].obj, gc_state);
                }
            }
        }
    }

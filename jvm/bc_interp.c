...
// WRITTEN: be careful with exceptions
static int
...
handle_new (u1 * bc, java_class_t * cls) {
	...
	java_class_t* target_cls = hb_resolve_class(idx, cls);
	if (!target_cls) {
		HB_ERR("Could not resolve class ref in %s\n", __func__);
		return -1;
	}
	
	obj_ref_t* oa = gc_obj_alloc(target_cls);
...
handle_newarray (u1 * bc, java_class_t * cls) {
	...
	var_t ret;
	obj_ref_t* oa = gc_array_alloc(type, count);
	if (!oa) {
		hb_throw_and_create_excp(EXCP_OOM);
		return -ESHOULD_BRANCH;
	}
...
handle_arraylength (u1 * bc, java_class_t * cls) {
	...
	if (v.obj->type != OBJ_ARRAY) {
		HB_ERR("Array does not match type reference: %s\n", __func__);
		return -1;
	}

	native_obj_t* aref = (native_obj_t*)v.obj->heap_ptr;

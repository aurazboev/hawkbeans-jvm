...
hb_resolve_class (u2 const_idx, java_class_t * src_cls)
{
    ...
    if (IS_RESOLVED(const_pool_entry)) {
        return (java_class_t*) MASK_RESOLVED_BIT(const_pool_entry);
    }

    CONSTANT_Class_info_t* const_class = (CONSTANT_Class_info_t*) const_pool_entry;
...
{
    if (!target_cls) {
        target_cls = hb_resolve_class(((CONSTANT_Methodref_info_t*)src_cls->const_pool[const_idx])->class_idx, src_cls);
        if (!target_cls) return NULL;
    }

    CONSTANT_Methodref_info_t* methodref_info = (CONSTANT_Methodref_info_t*)src_cls->const_pool[const_idx];
    CONSTANT_NameAndType_info_t* nameandtype_info = (CONSTANT_NameAndType_info_t*)src_cls->const_pool[methodref_info->name_and_type_idx];

    const char* method_name = hb_get_const_str(nameandtype_info->name_idx, src_cls);
    const char* method_desc = hb_get_const_str(nameandtype_info->desc_idx, src_cls);

    for (int i = 0; i < target_cls->methods_count; i++) {
        method_info_t* method = &target_cls->methods[i];
        const char* target_method_name = hb_get_const_str(method->name_idx, target_cls);
        const char* target_method_desc = hb_get_const_str(method->desc_idx, target_cls);
        if (!strcmp(method_name, target_method_name) && !strcmp(method_desc, target_method_desc)) {
            return method; 
        }
    }
    ...
    if (super_cls) {
        return hb_resolve_method(const_idx, src_cls, super_cls);
    }
